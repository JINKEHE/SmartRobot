...
public class SimpleRobot {
	...
	private final static double DIST = 11;

	// need to be calibrated
	private static final double WIDTH = 145;
	...
	public void forward() {
		...
		if (rightDist > DIST) {
			...
			server.send("frontDist < DIST");
			pilot.rotate(90);
		} else if (poseProvider.getPose().getHeading() % 180 == 0) {
			server.send("Forward: H");
			pilot.travel(H_MOVE);
		} else {
			server.send("Forward: W");
			pilot.travel(W_MOVE);
		}
	}
	...
	public void processInfo(double distance, int relativeHeading) {
		...
		switch (sensorHeading) {
			case 0:
				h_target += (int) Math.ceil(distance / H_MOVE);
			...
			case 180:
				...
				break;
			case 90:
				w_target += (int) Math.ceil(distance / W_MOVE);
	...
	/*
	 Rubbish:
			...
			for (int i = robot_grid[0] + 1; i < obstacle_grid[0]; i++) {
				gridMap.update(new int[] { i, robot_grid[1] }, false);
			}
		
			// gridMap.update(obstacle_grid, true);
			// str = "Scan right: " + distance;
	

	for (int i = obstacle_grid[0] + 1; i < robot_grid[0]; i++) {
		gridMap.update(new int[] { i, robot_grid[1] }, false);
	}
	gridMap.update(obstacle_grid, true);
	str = "Scan left: " + distance;

		targetY -= distance;
			targetX -= distance;
			obstacle_grid = poseToGrid(targetX, targetY);
			str = "Ob grid: " + obstacle_grid.toString();
			for (int i = obstacle_grid[1] + 1; i < robot_grid[1]; i++) {
				gridMap.update(new int[] { robot_grid[0], i }, false);
			}
			gridMap.update(obstacle_grid, true);
			str = "Scan up: " + distance;
