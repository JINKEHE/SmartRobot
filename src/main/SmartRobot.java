...
public class SmartRobot {
	...
	private void getInformation() {
		...
		if (needCalibrating == true && !readyToEnd) {
			...
			fourthCalibrate(lastRightDist, rightDistance);
		}
		// second calibrate (adjust distance)
		uSensorMotor.rotate(90);
	...
	private void fourthCalibrate(double lastRightDist, double rightDist) {
		double heading = getHeading();
		if (heading % 180 == 0) {
			lastRightDist %= W_MOVE;
			rightDist %= W_MOVE;
		} else {
			lastRightDist %= H_MOVE;
			rightDist %= H_MOVE;
		}
		firstCalibrate(lastRightDist, rightDist);
	}
