...
public class SmartRobot {
	...
	private static final double RESERVED_DIST_RIGHT = 14.5;
	// offset and diameter
	...
	private void buildMap(){
		...
		if ((heading % 180 != 0 && rightDistance > H_MOVE) || (heading % 180 == 0 && rightDistance > W_MOVE)) {
			...
			if ((robotW == 0 || robotW == W_GRID - 1) && frontDistance > H_MOVE){
				needCalibrating = true;
			}
			move(H_MOVE);
		} else {
			if ((robotH == 0 || robotH == H_GRID - 1) && frontDistance > W_MOVE) {
				needCalibrating = true;
			}
			move(W_MOVE);
		}
	...
	private void calibrate(double lastRightDist, double rightDist){
		double theta, tantheta, sintheta;
		if (getHeading() % 180 == 0) {
			tantheta = (rightDist-lastRightDist)/H_MOVE;			
		} else {
			tantheta = (rightDist-lastRightDist)/W_MOVE;
		}
		theta = Math.atan(tantheta)*180/Math.PI;
		...
		if(!robotIsAtCorner()){
			if (getHeading()%180==0){
				sintheta=(RESERVED_DIST_RIGHT-rightDist)/H_MOVE;
			} else{
				sintheta=(RESERVED_DIST_RIGHT-rightDist)/W_MOVE;
			}
			// -30
			lcd.clear();
